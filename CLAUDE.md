...
- **`sabre-flight-search`** (v6) — does the Sabre OAuth2 token exchange, then calls Bargain Finder Max. Both request and response are now built against verified real samples:
  ...
  - **Known gap — origin airport**: the Graba UI has no "flying from" selector at all — `originCode` has no real source yet and defaults to `"JNB"` if the caller omits it. Add a real origin selector before relying on this for genuine searches.
