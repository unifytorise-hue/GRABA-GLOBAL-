...
- **`sabre-flight-search`** (v4) — does the Sabre OAuth2 token exchange, then calls Bargain Finder Max. Split confidence:
  - **Token exchange encoding** (`getSabreToken()`) — `SABRE_CLIENT_ID`/`SABRE_CLIENT_SECRET` are the raw "User ID"/"Password" from Sabre Dev Studio's Applications page (format `V1:xxxxxxxxxxxx:DEVCENTER:EXT`). Standard OAuth2 Basic auth is a single `base64(id:secret)`, but Sabre has historically documented a Sabre-specific *double*-encoding scheme for this exact credential pair (`base64(base64(id) + ":" + base64(secret))`) — recalled with moderate confidence, not verified against current docs. Rather than guess, the function tries single-encoding first and automatically falls back to double-encoding if rejected; whichever worked is reported back as `tokenEncodingUsed` in the response.
